let express,exphbs,bcrypt,compare,sqlite3,open,cookieParser,grantAuthToken,searchUserFromAuthToken;_8e7‍.x([["dbPromise",()=>dbPromise]]);_8e7‍.w("express",[["default",["express"],function(v){express=v}]]);_8e7‍.w("express-handlebars",[["default",["exphbs"],function(v){exphbs=v}]]);_8e7‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}],["compare",["compare"],function(v){compare=v}]]);_8e7‍.w("sqlite3",[["default",["sqlite3"],function(v){sqlite3=v}]]);_8e7‍.w("sqlite",[["open",["open"],function(v){open=v}]]);_8e7‍.w("cookie-parser",[["default",["cookieParser"],function(v){cookieParser=v}]]);_8e7‍.w("./auth",[["grantAuthToken",["grantAuthToken"],function(v){grantAuthToken=v}],["searchUserFromAuthToken",["searchUserFromAuthToken"],function(v){searchUserFromAuthToken=v}]]);/* 
...
//Load database file upon first running the code
       const dbPromise = _8e7‍.a("open",open)({
    filename: "data.db",
    driver: _8e7‍.a("sqlite3",sqlite3).Database,
});;_8e7‍.j(["dbPromise"]);

const app = _8e7‍.a("express",express)();
var searchMovie;
var movieID;


app.engine("handlebars", _8e7‍.a("exphbs",exphbs)());
app.set("view engine", "handlebars");

//Middleware functions
app.use(_8e7‍.a("cookieParser",cookieParser)());
app.use(_8e7‍.a("express",express).urlencoded({ extended: false }));
app.use('/static', _8e7‍.a("express",express).static(__dirname + '/static'));
app.use('/img', _8e7‍.a("express",express).static(__dirname + '/img'));

app.use(async (req, res, next) =>{
    const { userToken } = req.cookies;
    if(!userToken) {return next();}
    try{
        const user = await _8e7‍.a("searchUserFromAuthToken",searchUserFromAuthToken)(userToken);
        req.user = user;
...
app.get("/requests", async (req, res) => {
    ...
    const db = await dbPromise;
    const movies = await db.get('SELECT movieTitle, movieLength, movieYear, movieRating FROM Movies WHERE movieTitle=?', searchMovie);
    _8e7‍.g.console.log("movie1", movieID)
    const category = await db.all(
        `SELECT
            addCategory,
            movieId
    ...
    _8e7‍.g.console.log('category', category);
    res.render("requests", {category, movies});
});
...
app.post('/register', async (req, res) =>{
    ...
    if(password===password2)
    {
        ...
        const passwordHash = await _8e7‍.a("bcrypt",bcrypt).hash(password, 10);
        ...
        try{
            ...
            const token = await _8e7‍.a("grantAuthToken",grantAuthToken)(user.id);
            _8e7‍.g.console.log('user registered', user);
            res.cookie('userToken', token);
            res.redirect('/');
        }
        catch (e) { return res.render('register', {error: e}, _8e7‍.g.console.log(e)); }//if something goes wrong during registration, error is passed.
    }
...
app.post('/login', async (req, res) =>{
    ...
    try{
        ...
        const passwordMatch = await _8e7‍.a("bcrypt",bcrypt).compare(password, existingUser.password);
        ...
        const token = await _8e7‍.a("grantAuthToken",grantAuthToken)(existingUser.id);
        res.cookie('userToken', token);
        res.redirect('/');
...
app.post('/addmovie', async (req, res) =>{
    ...
    try{
        ...
        searchMovie = await db.run('INSERT INTO Movies (movieTitle, movieLength, movieYear, movieRating) VALUES (?, ?, ?, ?);', req.body.movieTitle, req.body.movieLength, req.body.movieYear, req.body.movieRating);
